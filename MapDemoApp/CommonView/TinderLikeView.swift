...
// copied from https://github.com/nickypatson/TinderSwipeView/blob/master/testingTinderSwipe/ViewController.swift
...
class TinderCard: UIView {
    ...
    public init(frame: CGRect, matchRequestUserModel : MatchRequestUserModel) {
        super.init(frame: frame)
        setupView(matchRequestUserModel: matchRequestUserModel)
    }
    ...
    func setupView(matchRequestUserModel:MatchRequestUserModel) {
        ...
        if let imgUrl = matchRequestUserModel.headImageUrl {
            //test by kitahara
            //urlってこれで開けるんだっけ？？？無理では？
            profileImageView.image = UIImage(named:imgUrl)
        ...
        let attributedText = NSMutableAttributedString(string:matchRequestUserModel.name ?? "NoName", attributes: [.foregroundColor: UIColor.white,.font:UIFont.boldSystemFont(ofSize: 25)])
        attributedText.append(NSAttributedString(string: matchRequestUserModel.intro ?? "", attributes: [.foregroundColor: UIColor.white,.font:UIFont.systemFont(ofSize: 18)]))
